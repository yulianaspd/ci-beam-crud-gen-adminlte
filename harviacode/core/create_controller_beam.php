...
$string .= "\n\n    function __construct()
    {
        ...
        \$this->breadrumb   = '<li>'.anchor('','<i class=\"fa fa-dashboard\"></i> Home').'</li>
        <li class=\"active\">$c</li>';
        \$this->page_url    = site_url('$c_url');
        \$this->api_url     = site_url('api/".$c_url."');

        \$data['breadrumb']  = \$this->breadrumb;
...
$string .= "\n\n    public function index()
    {
        ...
        \$config['total_rows'] = \$this->" . strtolower($m) . "->total_rows(\$q);
        \$$c_url = \$this->" . strtolower($m) . "->get_limit_data(\$config['per_page'], \$start, \$q);
...
$string .= "\n\n    public function view(\$id){
        ...
        \$data = \$this->".strtolower($m)."->get_by_id(\$id);
...
$string .= "\n\n    function _updatedata(\$id = 0)
    {
        ...
        if (\$id > 0)
            \$this->form_validation->set_default(\$this->" .strtolower($m). "->get_by_id(\$id));

        if (\$this->form_validation->run())
        {
            if (\$id > 0)
            {
                \$this->".strtolower($m)."->update(\$id, \$this->form_validation->get_values());
            ...
            {
                \$this->".strtolower($m)."->insert(\$this->form_validation->get_values());
        ...
        \$this->breadrumb   = '<li>'.anchor('','<i class=\"fa fa-dashboard\"></i> Home').'</li>
        <li>'.anchor('$c_url','$c').'</li>
        <li class=\"active\">Form</li>';

        \$info['breadrumb']  = \$this->breadrumb;

        \$this->load->vars(\$info);

        \$this->data['panel_title']  = 'Form ".$c."';
...
$string .= "\n\n    public function delete(\$id)
    {
        \$row = \$this->".strtolower($m)."->get_by_id(\$id);

        if (\$row) {
            \$this->".strtolower($m)."->delete(\$id);
...
$string .= "\n\n\t\tforeach (\$this->" . strtolower($m) . "->get_all() as \$data) {
            \$kolombody = 0;
...
if ($export_word == '1') {
    $string .= "\n\n    public function word()
    {
        ...
        \$data = array(
            '" . $table_name . "_data' => \$this->" . strtolower($m) . "->get_all(),
            'start' => 0
        );
